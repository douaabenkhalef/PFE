...
function StatusBadge({ status, isSigned = false }) {
  const getStatusConfig = () => {
    if (isSigned) {
      return { label: 'Signée', color: 'bg-green-500/20 text-green-300 border-green-500/30' };
    }
    switch (status) {
      case 'accepted_by_company':
        return { label: 'En attente de validation', color: 'bg-yellow-500/20 text-yellow-300 border-yellow-500/30' };
...
const DetailsModal = ({ 
  ...
  const handleValidateClick = () => {
    if (!hasManagePermissions) {
      const errorMsg = "Vous n'avez pas la permission de valider des conventions. Veuillez contacter le Department Head.";
  ...
  const handleRejectClick = () => {
    if (!hasManagePermissions) {
      const errorMsg = "Vous n'avez pas la permission de refuser des conventions. Veuillez contacter le Department Head.";
      if (onPermissionError) onPermissionError(errorMsg);
      toast.error(errorMsg);
      return;
    }
  ...
  const handleSignatureClick = () => {
    if (!canSign) {
      const errorMsg = "Vous n'avez pas la permission d'ajouter une signature. Veuillez contacter le Department Head.";
      if (onPermissionError) onPermissionError(errorMsg);
      toast.error(errorMsg);
      return;
    }
  ...
  const handleStampClick = () => {
    if (!canAddStamp) {
      const errorMsg = "Vous n'avez pas la permission d'ajouter le cachet. Veuillez contacter le Department Head.";
      if (onPermissionError) onPermissionError(errorMsg);
      toast.error(errorMsg);
      return;
    }
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="bg-slate-800/60 rounded-xl p-5">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex items-center gap-2"><Calendar size={14} />Promotion: {application.student?.graduation_year || "—"}</div>
              </div>
              ...
              <div className="border-t border-slate-700 pt-5">
                {showRejectForm ? (
                  <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4">
                    ...
                    <textarea className="w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white resize-none min-h-[80px] mb-3" placeholder="Expliquez pourquoi cette demande est refusée..." value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} />
                  ...
                  <div className="flex gap-3">
                    {hasManagePermissions ? (
                      ...
                      <div className="w-full flex items-center justify-center gap-2 bg-red-500/20 text-red-300 px-5 py-2.5 rounded-lg text-sm">
                        <Lock size={16} /> Vous n'avez pas la permission de gérer les conventions.
                      </div>
                    )}
                  </div>
                )}
                  ...
                  {!isSigned && (
                    canSign ? (
                      ...
                      <div className="flex items-center gap-2 bg-red-500/20 text-red-300 px-4 py-2 rounded-lg text-sm">
                        <Lock size={14} /> Permission requise pour signer
                      </div>
                    )
                  )}
                  {!hasStamp && (
                    canAddStamp ? (
                      ...
                      <div className="flex items-center gap-2 bg-red-500/20 text-red-300 px-4 py-2 rounded-lg text-sm">
                        <Lock size={14} /> Permission requise pour le cachet
                      </div>
                    )
                  )}
                </div>
...
export default function CoDeptValidations() {
  ...
  // Récupérer le statut de signature pour une convention
  const fetchSignatureStatus = async (appId) => {
    try {
      ...
      console.error("Erreur récupération statut signature:", err);
    }
  };
  ...
  const fetchApplications = async () => {
    setLoading(true);
    try {
      // Récupérer les conventions en attente et validées
      ...
      if (pendingData.success) {
        ...
        // Récupérer les statuts de signature
  ...
  const handleValidate = async (applicationId) => {
    ...
    try {
      ...
      if (res.status === 403) {
        const errorMsg = data.error || data.detail || "Vous n'avez pas les permissions pour valider cette convention.";
        setPermissionError(errorMsg);
        setTimeout(() => setPermissionError(null), 5000);
        toast.error(errorMsg);
        return;
  ...
  const handleReject = async (applicationId, reason) => {
    ...
    try {
      ...
      if (res.status === 403) {
        const errorMsg = data.error || data.detail || "Vous n'avez pas les permissions pour refuser cette convention.";
        setPermissionError(errorMsg);
        setTimeout(() => setPermissionError(null), 5000);
        toast.error(errorMsg);
        return;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {permissionError && (
            <div className="flex items-center gap-3 bg-orange-500/10 backdrop-blur-lg border border-orange-500/30 rounded-xl p-4 mb-6">
              ...
              <div>
                <p className="text-orange-300 font-medium">Permission refusée</p>
                <p className="text-orange-400/80 text-sm">{permissionError}</p>
                <p className="text-orange-400/60 text-xs mt-1">💡 Veuillez contacter votre Department Head pour demander les permissions nécessaires.</p>
              </div>
            </div>
          )}
          ...
          <div className="mb-8">
            ...
            <p className="text-white/60">Gérez les conventions de stage</p>
          </div>
          ...
          <div className="flex gap-2 mb-6 border-b border-white/20 pb-2">
            ...
            >
              Conventions en attente ({counts.pending})
            </button>
            <button
            ...
            >
              Prêtes à signer ({counts.readyToSign})
            </button>
            <button
            ...
            >
              Signées ({counts.signed})
            </button>
          </div>

          {loading ? (
            <div className="flex items-center justify-center py-24 text-white/50">
              <Loader2 size={24} className="animate-spin mr-3" /> Chargement des conventions...
            </div>
            ...
            <div className="bg-white/5 rounded-xl p-16 text-center border border-white/10">
              ...
              <p className="text-white/50">
                {activeTab === 'pending' ? 'Aucune convention en attente de validation.' : 
                 activeTab === 'readyToSign' ? 'Aucune convention prête à signer.' : 
                 'Aucune convention signée.'}
              </p>
            </div>
          ) : (
