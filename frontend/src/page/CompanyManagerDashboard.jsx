...
import { CheckCircle, XCircle, Clock, Building2, Mail, User, Briefcase } from "lucide-react";
...
const CompanyManagerDashboard = () => {
  ...
  const fetchPendingHiringManagers = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8000/api/company/pending-hiring-managers/", {
  ...
  const handleApprove = async (managerId, managerName) => {
    ...
    try {
      const token = localStorage.getItem("access_token");
      const response = await fetch(`http://localhost:8000/api/company/approve-hiring-manager/${managerId}/`, {
        method: "POST",
        headers: { "Authorization": `Bearer ${token}` }
      });
      const data = await response.json();
      if (data.success) {
        toast.success(`Hiring manager ${managerName} a été approuvé`);
        fetchPendingHiringManagers();
      } else {
  ...
  const handleReject = async (managerId, managerName) => {
    ...
    try {
      const token = localStorage.getItem("access_token");
      const response = await fetch(`http://localhost:8000/api/company/reject-hiring-manager/${managerId}/`, {
        method: "POST",
        headers: { "Authorization": `Bearer ${token}` }
      });
      const data = await response.json();
      if (data.success) {
        toast.success(`Hiring manager ${managerName} a été refusé`);
        fetchPendingHiringManagers();
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 to-indigo-900">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-8">
          <h2 className="text-3xl font-bold text-white mb-2">Hiring Managers en attente</h2>
          <p className="text-white/60">Validez ou refusez les demandes d'inscription des Hiring Managers</p>
        </div>
        ...
        {pendingHiringManagers.length === 0 ? (
          <div className="bg-white/10 backdrop-blur-lg rounded-xl p-12 text-center border border-white/20">
            ...
            <p className="text-white/60">Tous les hiring managers ont été traités.</p>
          </div>
        ) : (
            ...
            {pendingHiringManagers.map((manager) => (
              ...
              >
                ...
                <div className="space-y-2 mb-4">
                  ...
                  {manager.description && (
                    <p className="text-white/80 text-sm line-clamp-2">{manager.description}</p>
                  )}
                  {manager.location && (
                    <p className="text-white/60 text-sm">📍 {manager.location}</p>
                  )}
                  {manager.industry && (
                    <p className="text-white/60 text-sm">🏭 {manager.industry}</p>
                  )}
                </div>
