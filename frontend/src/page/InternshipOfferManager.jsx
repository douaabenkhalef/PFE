...
function OfferForm({ initial, onSubmit, submitLabel, submitting }) {
  ...
  const setBool = (k) => (e) => setF((p) => ({ ...p, [k]: e.target.value === 'true' }));
  const setFile = (k) => (e) => {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(f); }} className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-2xl">
      <div className="md:col-span-2">
        ...
        <input className={inp} placeholder="e.g. Mobile Application Developer Intern" value={f.title} onChange={setField('title')} required />
      </div>
      <div className="md:col-span-2">
        <label className="block text-xs text-slate-400 mb-1 font-medium">Description <span className="text-red-400">*</span></label>
        <textarea className={`${inp} min-h-[90px] resize-y`} placeholder="Describe the internship role..." value={f.description} onChange={setField('description')} required />
      </div>
      <div>
        <label className="block text-xs text-slate-400 mb-1 font-medium">Wilaya <span className="text-red-400">*</span></label>
        <select className={inp} value={f.wilaya} onChange={setField('wilaya')} required>
      ...
      <div>
        ...
        <select className={inp} value={f.internship_type} onChange={setField('internship_type')} required>
      ...
      <div>
        ...
        <input className={inp} placeholder="e.g. 3 months" value={f.duration} onChange={setField('duration')} required />
      </div>
      <div>
        <label className="block text-xs text-slate-400 mb-1 font-medium">Start Date <span className="text-red-400">*</span></label>
        <input type="date" className={inp} value={f.start_date} onChange={setField('start_date')} required />
      </div>
      <div className="md:col-span-2">
        ...
        <input className={inp} placeholder="e.g. Flutter, Firebase, REST API" value={f.required_skills} onChange={setField('required_skills')} />
      </div>
      <div className="md:col-span-2">
      ...
      <div>
        ...
        <select className={inp} value={String(f.is_active)} onChange={setBool('is_active')}>
      ...
      <div>
        ...
        <input type="date" className={inp} value={f.deadline} onChange={setField('deadline')} required />
      </div>
...
export default function InternshipOfferManager() {
  ...
  const loadOffers = async (s = search, t = fType, a = fActive) => {
    ...
    try {
      ...
      if (data.success) { 
        ...
        showMsg('error', data.message || 'Failed to load offers.');
      }
    } catch (e) { 
  ...
  const handleCreate = async (formData) => {
    ...
    Object.keys(formData).forEach(key => {
      if (key === 'image' && formData.image instanceof File) {
        ...
        formDataToSend.append(key, formData.required_skills);
      } else if (key === 'is_active') {
        formDataToSend.append(key, formData.is_active === true || formData.is_active === 'true');
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { 
        showMsg('success', 'Offer created successfully!'); 
        setActiveTab('list');
        loadOffers();
      } else {
        showMsg('error', data.message || 'Failed to create offer.');
      }
    } catch (e) { 
      showMsg('error', `Network error: ${e.message}`); 
    } finally { 
  ...
  const viewOffer = async (id) => {
    try {
      const res = await fetch(`${BASE_URL}/company/offers/${id}/`, { headers: authHeaders() });
      const data = await res.json();
      if (data.success) {
        setModal(data.offer);
      } else {
        showMsg('error', data.message || 'Failed to load details.');
      }
    } catch (e) { 
      showMsg('error', `Network error: ${e.message}`); 
    }
  ...
  const openEdit = (offer) => {
    setEditOffer({
      ...
      is_active: offer.is_active === true || offer.is_active === 'true' || offer.is_active === true,
    });
    setActiveTab('edit');
    setMsg(null);
  };
  ...
  const handleUpdate = async (formData) => {
    ...
    Object.keys(formData).forEach(key => {
      if (key === 'image' && formData.image instanceof File) {
        ...
        const isActiveValue = formData.is_active === true || formData.is_active === 'true';
        formDataToSend.append(key, isActiveValue);
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { 
        showMsg('success', 'Offer updated successfully!'); 
        setEditOffer(null); 
        setActiveTab('list');
        loadOffers();
      } else {
        showMsg('error', data.message || data.error || 'Failed to update offer.');
      }
    } catch (e) { 
      showMsg('error', `Network error: ${e.message}`); 
    } finally { 
  ...
  const handleDelete = async (offer) => {
    ...
    setMsg(null);
    try {
      const res = await fetch(`${BASE_URL}/company/offers/${offer.id}/delete/`, { 
        method: 'DELETE', 
        headers: authHeaders() 
      });
      const data = await res.json();
      if (data.success) { 
        showMsg('success', `"${offer.title}" deleted successfully.`); 
        loadOffers();
    ...
    <div className="min-h-screen flex">
            ...
            <>
              ...
              {loading ? (
                <div className="flex items-center justify-center py-24 text-white/50">
                  <Loader2 size={24} className="animate-spin mr-3" /> Loading offers...
                </div>
              ) : offers.length === 0 ? (
                <div className="text-center py-24 text-white/50">
                  ...
                  <p className="text-sm mt-1">
                    Create your first offer using the{' '}
                    <button onClick={() => setActiveTab('create')} className="text-purple-400 hover:underline">
                      New Offer
                    </button>{' '}
                    button.
                  </p>
                        ...
                        {offers.map((o) => (
                          <tr key={o.id} className="hover:bg-white/5 transition">
                            ...
                            <td className="px-4 py-3">
                              <span className="bg-blue-900/60 text-blue-300 px-2 py-0.5 rounded text-xs font-medium">
                                {o.internship_type}
                              </span>
                            </td>
                            ...
                            <td className="px-4 py-3">
                              <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                                o.is_active ? 'bg-green-900/60 text-green-300' : 'bg-red-900/60 text-red-300'
                              }`}>
          ...
          {activeTab === 'edit' && editOffer && (
            <>
              ...
              <p className="text-white/50 text-sm mb-6">
                Editing: <span className="text-white/80 font-medium">{editOffer.title}</span>
              </p>
              <OfferForm 
                initial={editOffer} 
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={() => setModal(null)}>
          <div className="bg-[#1e293b] border border-slate-700 rounded-2xl w-full max-w-lg p-8 relative shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setModal(null)} className="absolute top-4 right-4 text-slate-500 hover:text-white transition">
              <X size={20} />
            </button>
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              {[
                ['Type', modal.internship_type],
                ['Wilaya', modal.wilaya],
                ['Duration', modal.duration],
                ['Start Date', modal.start_date || '—'],
                ['Status', modal.is_active ? 'Active' : 'Inactive'],
                ['Created', modal.created_at || '—']
              ].map(([label, value]) => (
            ...
            {modal.required_skills?.length > 0 && (
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {modal.required_skills.map((s) => (
                    <span key={s} className="bg-indigo-900/60 text-indigo-300 text-xs px-2.5 py-1 rounded-full">
                      {s}
                    </span>
                  ))}
                </div>
            ...
            <div className="flex gap-3 pt-2 flex-wrap">
              <button 
                onClick={() => { setModal(null); openEdit(modal); }} 
                className="flex items-center gap-2 bg-yellow-600/20 hover:bg-yellow-600/30 text-yellow-300 px-4 py-2 rounded-lg text-sm transition"
              >
                <Pencil size={14} /> Edit
              </button>
              <button 
                onClick={() => { setModal(null); handleDelete(modal); }} 
                className="flex items-center gap-2 bg-red-600/20 hover:bg-red-600/30 text-red-300 px-4 py-2 rounded-lg text-sm transition"
              >
      ...
      <style>{`
        @media (max-width: 768px) {
          .ml-64 { margin-left: 220px !important; }
          .max-w-7xl { padding-left: 1rem; padding-right: 1rem; }
          .flex.items-center.justify-between { flex-direction: column; align-items: flex-start !important; gap: 0.75rem; }
          .overflow-x-auto { overflow-x: auto; }
          table { min-width: 800px; }
          .grid-cols-1.md\\:grid-cols-2 { grid-template-columns: 1fr !important; }
          .flex.gap-3 { flex-direction: column; align-items: stretch; }
          .flex.gap-3 select,
          .flex.gap-3 .relative,
        ...
        @media (max-width: 580px) {
          .ml-64 { margin-left: 200px !important; }
          .px-4 { padding-left: 0.75rem; padding-right: 0.75rem; }
          .py-8 { padding-top: 1rem; padding-bottom: 1rem; }
          .text-2xl.font-bold { font-size: 1.2rem; }
        }
        
        @media (max-width: 480px) {
          .ml-64 { margin-left: 180px !important; }
          .w-40, .w-36 { width: 100% !important; }
        }
        ...
        body.light-mode .text-white\\/90 { color: #1a1a2e !important; }
        body.light-mode .text-white\\/50,
        body.light-mode .text-white\\/60,
        body.light-mode .text-white\\/40,
        body.light-mode .text-white\\/30 { color: #666 !important; }
        body.light-mode .text-purple-300\\/60 { color: #8D23D4 !important; opacity: 0.7; }
        body.light-mode .bg-white\\/10 { background: rgba(141, 35, 212, 0.08) !important; }
        body.light-mode .border-white\\/10,
        body.light-mode .border-white\\/20 { border-color: rgba(141, 35, 212, 0.15) !important; }
        body.light-mode .bg-purple-600\\/30 { background: rgba(141, 35, 212, 0.15) !important; }
        body.light-mode .text-purple-300 { color: #8D23D4 !important; }
        body.light-mode .hover\\:bg-white\\/10:hover { background: rgba(141, 35, 212, 0.1) !important; }
        ...
        body.light-mode select option { background: white !important; color: #1a1a2e !important; }
        body.light-mode .bg-indigo-600 { background: #8D23D4 !important; }
        body.light-mode .bg-indigo-600:hover { background: #6B21A5 !important; }
        body.light-mode .bg-purple-600 { background: #8D23D4 !important; }
        body.light-mode .bg-purple-600:hover { background: #6B21A5 !important; }
        body.light-mode .bg-\\[\\#1e293b\\] {
          background: white !important;
          border-color: rgba(141, 35, 212, 0.2) !important;
        }
        body.light-mode .bg-\\[\\#1e293b\\] .text-white { color: #1a1a2e !important; }
        body.light-mode .bg-\\[\\#1e293b\\] .text-slate-500,
        body.light-mode .bg-\\[\\#1e293b\\] .text-slate-300 { color: #666 !important; }
        body.light-mode .bg-slate-800 { background: rgba(0, 0, 0, 0.05) !important; }
        body.light-mode .bg-slate-800 .text-white { color: #1a1a2e !important; }
        body.light-mode .bg-green-500\\/10 { background: rgba(5, 150, 105, 0.1) !important; }
        body.light-mode .bg-red-500\\/10 { background: rgba(220, 38, 38, 0.1) !important; }
        body.light-mode .text-green-300 { color: #059669 !important; }
        body.light-mode .text-red-300 { color: #dc2626 !important; }
        body.light-mode .bg-blue-900\\/60 { background: rgba(141, 35, 212, 0.1) !important; }
        body.light-mode .text-blue-300 { color: #8D23D4 !important; }
        body.light-mode .bg-green-900\\/60 { background: rgba(5, 150, 105, 0.1) !important; }
        body.light-mode .bg-red-900\\/60 { background: rgba(220, 38, 38, 0.1) !important; }
        body.light-mode .text-indigo-400 { color: #8D23D4 !important; }
