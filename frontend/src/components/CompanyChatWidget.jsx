  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 border-t border-slate-700 flex gap-2 flex-shrink-0 bg-slate-800/50">
        <input
          type="file"
          id="company-chat-file-input"
          accept="image/*,.pdf,.doc,.docx,.txt"
          className="hidden"
          onChange={handleFileSelect}
        />
        <label
          htmlFor="company-chat-file-input"
    ...
    >
      ...
      <div className="flex-1 text-left">
        ...
        <p className="text-white/40 text-xs">
          {member.sub_role === 'company_manager' ? 'Company Manager' : 'Hiring Manager'}
          {isCurrentUser && ' (Vous)'}
        </p>
      </div>
    </button>
          ...
          availableMembers.length === 0 ? (
            ...
            availableMembers.map(member => (
              <CompanyMemberItem
                ...
                isCurrentUser={member.is_current}
              />
            ))
          )
        )}
