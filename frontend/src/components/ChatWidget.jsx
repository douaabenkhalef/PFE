...
import { 
  ...
  GraduationCap, Search, User, MessageSquare,
  Clock, CheckCheck, ArrowLeft, Loader2, Paperclip, File
} from 'lucide-react';
...
const ChatWindow = ({ 
  ...
  const connectWebSocket = useCallback(() => {
    ...
    if (conversation.type === 'group') {
      wsUrl = `${WS_BASE}/ws/chat/${conversation.room_name}/?token=${accessToken}`;
    } else {
      wsUrl = `${WS_BASE}/ws/private-chat/?token=${accessToken}`;
    }
  ...
  const sendMessage = () => {
    ...
    const messageData = {
      type: conversation.type === 'private' ? 'private_message' : 'message',
      message: inputMessage.trim(),
      message_type: 'text'
    };
    
    if (conversation.type === 'private' && conversation.user_id) {
      messageData.receiver_id = conversation.user_id;
    }
...
// Conversation list item (for both group and private chats)
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 text-left min-w-0">
        ...
        {conversation.type === 'group' && conversation.member_count && (
...
// Team member item (sans bouton mauve)
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 text-left">
        ...
        <p className="text-white/40 text-xs">{member.sub_role === 'admin' ? 'Department Head' : 'Co Department Head'}</p>
      </div>
    </button>
  );
};
...
const ChatWidget = ({ university, companyMode = false, companyId = null, internshipId = null, onClose: externalOnClose }) => {
  const { user } = useAuth();
  ...
  // Auto-open chat when internshipId is provided (for students)
  useEffect(() => {
  ...
  // Load existing private conversations from chat history
  const loadExistingConversations = useCallback(async () => {
    try {
      ...
      if (data.success && data.conversations) {
        ...
        // Update lastMessages for each conversation
        convos.forEach(conv => {
  ...
  // Fetch team members (university admins)
  ...
  // Load all conversations (group + existing private chats)
  const loadAllConversations = useCallback(async () => {
    setLoading(true);
    try {
      const universityName = university?.replace(/\s/g, '_') || 'Universite';
      const roomName = universityName.replace(/\s/g, '_');
      
      // Create group conversation
      const groupConversation = {
        id: `group_${roomName}`,
        name: `Chat ${university || 'Université'}`,
        type: 'group',
        room_name: roomName,
      ...
      // Load existing private conversations
      const privateConversations = await loadExistingConversations();
      
      // Combine: group first, then private chats
  ...
  }, [university, teamMembers.length, loadExistingConversations]);

  // For admin roles: fetch team members and conversations
  useEffect(() => {
  ...
  const handleStartPrivateChat = async (member) => {
    ...
    if (existingConv) {
      // Reset unread count when opening existing conversation
      setConversations(prev => prev.map(conv => 
  ...
  // Get members that don't have an existing chat yet (for Team tab)
  const getAvailableMembers = () => {
    ...
    return teamMembers.filter(member => {
      ...
      return matchesSearch && member.id !== user?.id; // Afficher tous les membres même ceux avec qui on a déjà discuté
    });
  };

  const availableMembers = getAvailableMembers();

  // For company mode or student mode, use the original simple chat
  if (companyMode || (user?.role === 'student' && !internshipId)) {
    ...
    if (!isOpen) {
      return (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 bg-gradient-to-r from-purple-600 to-pink-600 text-white p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300"
        >
    ...
    return (
      <div className="fixed bottom-6 right-6 z-50 w-96 h-[500px] bg-[#1e293b] rounded-2xl shadow-2xl border border-slate-700 overflow-hidden flex flex-col">
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 p-3 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleCloseChat} className="text-white/80 hover:text-white">
              <X size={16} />
            </button>
          </div>
        </div>
        {!isMinimized && (
          <div className="flex-1 flex flex-col">
            <div className="flex-1 overflow-y-auto p-3 bg-slate-900/50">
              <div className="flex flex-col items-center justify-center h-full text-white/40">
                <MessageCircle size={32} className="mb-2 opacity-30" />
                <p className="text-sm">Chat non disponible pour ce mode</p>
              </div>
            </div>
          </div>
        )}
  ...
  // For admin (dept head / co dept head)
  if (!isOpen) {
    return (
      <button
        onClick={handleOpenChat}
